// js/miembros.js
// Script exclusivo para la página miembros.html - Versión FINAL CRUD
...
// --- TAREA 1: Cargar la grilla (¡CORREGIDO PARA AÑADIR ELIMINAR!) ---
async function cargarMiembrosGrid() {
    ...
    try {
        ...
        miembros.forEach(miembro => {
            ...
            const claseEstado = estado.toLowerCase() === 'activo' ? 'active' : 'expired';
            ...
            card.innerHTML = `
                ...
                <p><strong>Membresía:</strong> ${miembro.membership_type || 'N/A'}</p>
            ...
            // 2. Listener para ELIMINAR (¡NUEVO!)
            card.querySelector('.btn-delete-card').addEventListener('click', () => {
                ...
                if (confirm(`¿Estás seguro de que quieres eliminar a ${miembro.first_name} ${miembro.last_name}?`)) {
                    eliminarMiembro(miembro.id); // Llamamos a la nueva función
                }
            });
        ...
        if (miembros.length === 0) {
            grid.innerHTML = '<p>No se encontraron miembros.</p>';
        }
    } catch (error) {
...
function setupCreateModalToggles() {
    ...
    // Asumiendo IDs sincronizados con -crear
...
function setupCreateFormSubmitListener() {
    ...
    phone: formData.get('phone'), // Nuevo
    
    // CAMPOS QUE LA API REQUIERE
    address: formData.get('address'), // Nuevo
    discount_type: formData.get('discount_type'), // Nuevo
    date_of_birth: formData.get('date_of_birth'), // Nuevo
    
    // DNI NO ES REQUERIDO POR LA API PERO LO MANTENEMOS POR NEGOCIO
...
function setupEditFormSubmitListener() {
    ...
    address: formData.get('address'), // <-- Agregado
    member_type: formData.get('member_type'), // <-- Agregado
    discount_type: formData.get('discount_type'), // <-- Agregado
    date_of_birth: formData.get('date_of_birth') // <-- Nombre API: date_of_birth
};
...
// En miembros.js, dentro de abrirModalEdicion:
function abrirModalEdicion(miembro) {
    ...
    // --- LÍNEAS AGREGADAS PARA RELLENAR EL FORMULARIO ---
...
async function eliminarMiembro(memberId) {
    try {
        const respuesta = await fetch(`${API_BASE_URL}/members/${memberId}`, {
            method: 'DELETE' // Usamos el método DELETE que espera FastAPI
        });
        ...
        // Si la eliminación fue exitosa (200, 204, etc.):
        alert('Miembro eliminado con éxito.');
        cargarMiembrosGrid(); // Recargar la grilla para que el miembro desaparezca
